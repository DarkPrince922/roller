...
Основной способ управления — постоянная клавиатура с кнопками (без `/`),
которую показывает `/start`: ▶️ Старт, ⏹ Стоп, 📊 Статус, 📋 Список,
🎯 Найденные, 🧪 Калибровка, 🔀 Стратегия, 🌐 Цель, 🧦 Прокси, ⚙️ Лимиты,
🧾 Логи. Доступ только для chat_id из `TELEGRAM_ALLOWED_CHAT_IDS`.

Для разделов, которым нужен ввод (Цель/Прокси/Лимиты), кнопка открывает
inline-меню: там можно переключить готовые опции одним тапом (например,
очистить цель, проверить прокси) или нажать «✏️ изменить» — бот попросит
прислать значение обычным текстовым сообщением (CIDR/ASN списком через
запятую, `host:port:user:pass` для прокси, число для лимита) и применит
его сразу, без слэш-команд. Каждый такой запрос можно отменить кнопкой
«❌ Отмена». `/list` дополнительно показывает кнопки «🧹 Промахи» и «🗑 Все»
для массового освобождения.

Для скриптов/привычки руками команды тоже работают: `/hunt`, `/stop`,
`/status`, `/target [cidr <list>|asn <list>|clear]`,
`/strategy [release_immediately|hold_window]`, `/calibrate [n]`,
`/proxy [host:port:user:pass|test]`, `/list`, `/release <id|all|misses>`,
`/found`, `/limits [<поле> <значение>]`, `/logs [n]`.
